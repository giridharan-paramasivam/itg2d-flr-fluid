...
def init_linmats(kx,ky,pars,n_hyper=1):    
    ...
    lm[:,0,0]=-1j*D*kpsq**n_hyper-1j*sigk*H/kpsq**2
    lm[:,0,1]=(kapn+kapt)*ky
    lm[:,1,0]=-kapb*ky/Lk
    lm[:,1,1]=(-(kapb-kapn)*ky-(kapn+kapt)*ky*kpsq)/Lk-1j*D*kpsq**n_hyper-1j*sigk*H/kpsq**2
...
def gam_max(kx, ky, kapn, kapt, kapb, D, H, n_hyper=1):
