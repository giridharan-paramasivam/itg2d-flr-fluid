...
dtshow=1.0
gammax=gam_max(kx,ky,kapn,kapt,kapb,Gamma,nu,H,n_hyper)
...
fname = output_dir + f'out_PV_hyper_kapt_{str(kapt).replace(".", "_")}_nu_{format_exp(nu)}_H_{format_exp(H)}.h5'
...
lm[:,1,0]=(-(kapn+kapt)*1j*ky+Gamma*1j*ky*(kapn+kapt)*kpsq).get()
lm[:,1,1]=(Gamma*1j*ky*kapb-nu*kpsq**n_hyper-sigk*H/kpsq**2).get()
...
def rhs_nl(t,y):
    ...
    nl_dia = kx**2*rft2(dxphi*dyP) - ky**2*rft2(dyphi*dxP) + kx*ky*rft2(dyphi*dyP - dxphi*dxP)
    dPhikdt[:] += nl_dia / Lk

    dPkdt[:]+=rft2(dyphi*dxP-dxphi*dyP)+Gamma*nl_dia
...
if(wecontinue):
    ...
    save_data(fl,'params',ext_flag=False,Npx=Npx,Npy=Npy,Lx=Lx,Ly=Ly,kapn=kapn,kapt=kapt,kapb=kapb,Gamma=Gamma,nu=nu,H=H,n_hyper=n_hyper,gammax=gammax)
...
r=gsol(rhs_nl, t0, zk, t1, lm, dtstep, callbacks=cbs, sv="etdrk4cp", maxstep=1.0, tol=1e-8)
r.run()
fl.close()
